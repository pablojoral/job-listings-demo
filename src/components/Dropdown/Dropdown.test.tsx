...
describe('Dropdown', () => {
  it('renders the native picker', async () => {
    const { getByTestId } = await render(
      <Dropdown options={options} selectedValue={null} onChange={jest.fn()} placeholder="All categories" testID="dropdown" />,
    );
  ...
  it('reports the picked value through onChange', async () => {
    ...
    const { getByTestId } = await render(
      <Dropdown options={options} selectedValue={null} onChange={onChange} placeholder="All categories" testID="dropdown" />,
    );
  ...
  it('reports null when the placeholder option is picked', async () => {
    const onChange = jest.fn();
    const { getByTestId } = await render(
      <Dropdown options={options} selectedValue="design" onChange={onChange} placeholder="All categories" testID="dropdown" />,
    );
