...
interface IconProps {
  ...
  /** Fill closed shapes with the icon color instead of stroke-only rendering. */
  filled?: boolean;
  testID?: string;
}

/** The app's themed icon. Renders a small set of hand-authored SVG glyphs. */
export const Icon = ({ name, size = 'icon-size-md', color = 'font-primary', filled = false, testID }: IconProps) => {
  const { pixelSize, colorValue } = useIconTheme(size, color);
  const fillValue = filled ? colorValue : 'none';
